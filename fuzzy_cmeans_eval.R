fuzzy_cmeans_eval <- function(data, target_col, length_test = 20, c = NULL, m = NULL) {
  ...
  for (ci in c_values) {
    for (mi in m_values) {
      ...
      fit <- suppressWarnings(lm(y_train ~ ., data = as.data.frame(train_data_with_membership)))
      ...
      predictions <- suppressWarnings(predict(fit, newdata = as.data.frame(test_data_with_membership)))
