fuzzy_cmeans_eval_lasso <- function(data, target_col, length_test = 20, c = NULL, m = NULL) {
  ...
  for (ci in c_values) {
    for (mi in m_values) {
      suppressWarnings({
        ...
        ridge_fit <- cv.glmnet(as.matrix(train_data_with_membership), y_train, alpha = 1, lambda = lambdas)
        best_lambda_current <- ridge_fit$lambda.min
        
        fit <- glmnet(as.matrix(train_data_with_membership), y_train, alpha = 1, lambda = best_lambda_current)
      ...
      if (rmse < best_rmse) {
        ...
        best_c <- ci
        best_m <- mi
        best_lambda <- best_lambda_current
  ...
  if (is.null(c) | is.null(m)) {
    output <- sprintf(
      ...
      "Fuzzy C-Means Evaluation Lasso Results",
      strrep("=", 30),
      "Best Parameters:",
      best_c,
      best_m,
